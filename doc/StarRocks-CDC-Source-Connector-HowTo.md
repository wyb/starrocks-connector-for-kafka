...
| `source.snapshot.mode` | String | `initial` | Whether to take an initial snapshot before streaming changes. One of `initial`, `no_snapshot`. **`no_snapshot` cannot be combined with `source.nontrackable.policy=resnapshot`** — that pairing is rejected at startup with a `ConfigException`, because `resnapshot` rebuilds a table's position from a fresh snapshot and would silently drop changes with snapshots disabled. With `no_snapshot`, streaming starts at the version current when the task first starts; a restart before the first change window is durable resumes from the bookmark that earlier start pinned, as long as the connector's holder still references it (it expires with `source.bookmark.ttl.ms`). |
...
source.poll.interval.ms=5000
source.bookmark.ttl.ms=604800000
...
The Kafka record key is a `Struct` built from the table's key columns -- the primary key of a PRIMARY KEY table, the aggregate key of an AGGREGATE KEY table, the sort key of a DUPLICATE KEY table; `information_schema.columns.COLUMN_KEY` marks them for every model -- using the same key schema for every record from that table. A DUPLICATE KEY sort key is not unique, so several rows can share one Kafka key; see [Limitations](#limitations) for what that means for compaction and tombstones. A table with no key column produces records with a `null` key.
...
- **Exactly-once is available, but only without a snapshot.** Set `source.snapshot.mode=no_snapshot` and run on a distributed worker with `exactly.once.source.support=enabled` (Kafka 3.3+): Connect then writes each poll's records and their offset in one producer transaction, so a crash before the commit aborts the batch and a `read_committed` consumer never sees it. With `source.snapshot.mode=initial` the connector reports `UNSUPPORTED` and the worker refuses to run it exactly-once. Snapshot rows carry `snapshot_done=false` on purpose, so a restart redoes the snapshot whole rather than resuming a half-delivered one — correct for at-least-once, but under exactly-once every redone row is a duplicate the consumer has already seen. Note that `no_snapshot` starts streaming from the version current at the first start; a restart before any window has committed resumes from the bookmark that start pinned -- the task adopts every bookmark its holder still references and takes the oldest -- so the changes in between are read, not skipped. Only when the reference list cannot be read at start, or the bookmark has expired, does the task log a warning and re-establish "now".

- **Resetting offsets also releases the FE references.** With the connector stopped, `DELETE /connectors/{name}/offsets` makes `alterOffsets` release every bookmark the tasks' holder still references on each table Connect has an offset stored for -- and on every configured table when it has none stored at all -- so the next start begins at the current version instead of resuming from a pinned one. A table that never reached a durable offset (under `no_snapshot`, before its first change window) is not in a `DELETE` that has other tables stored; reset it with `PATCH`, naming its partition with a null offset. A connector re-created under an old name inherits that name's references; one `DELETE` clears them. All of this needs a configuration that parses and an FE that answers; if a release fails the reset is refused and nothing is written. `PATCH` with explicit offsets is validated only: `bookmark_id` must be a non-negative number and `snapshot_done` a boolean, for a table the configuration captures.
- **At-least-once delivery (the default).** A table's older bookmarks are only ever released from the task's `commit()` callback, and only below the offset the task reads back out of Connect's own offset store — never eagerly during `poll()`, and never on the strength of `commit()` having been called, since Connect invokes it even when no flush happened. If the task crashes and restarts before that flush completes, the same CHANGES window (or the same snapshot) is replayed on restart, producing duplicate records. Downstream consumers should apply changes idempotently keyed on the record key, taking the records in Kafka partition order — a replay re-delivers the same records in the same order, so last-write-wins per key converges — or read from a compacted topic. Do **not** deduplicate by requiring `source.row_version` to increase: it is not comparable across record types (see [`row_version` across record types](#row_version-across-record-types)), and both halves of an `UPDATE` legitimately share one value.
...
- **`source.nontrackable.policy`** controls what happens when a CHANGES window can no longer be read (see [Limitations](#limitations) for causes):
  - `fail` (default): the task throws and stops. An operator must intervene — for example by raising `source.bookmark.ttl.ms`, fixing the underlying cause, or switching this table to `resnapshot`.
...
3. **DUPLICATE KEY / AGGREGATE KEY tables carry a non-unique key and are insert-only.** Their Kafka key is the sort key or the aggregate key. A DUPLICATE KEY sort key admits duplicates, so several rows share one Kafka key: partitioning and per-key ordering hold, but log compaction would drop rows that are not duplicates, and with `source.tombstones.on.delete=true` a tombstone would delete every row sharing the deleted row's key. They are also only trackable for pure insert workloads: a `DELETE` against a DUPLICATE KEY table, for instance, makes that CHANGES window non-trackable (StarRocks reports `CDC-ERROR-1 (CHANGE_NOT_TRACKABLE): CDC for DUP_KEYS does not support delete`). AGGREGATE KEY tables report pre-aggregated values (per the table's own aggregate functions), not raw inserted rows.
...
5. **A `resnapshot` can redeliver already-emitted changes.** The failing window's own records are dropped from the batch before the policy runs, but records from earlier windows are already in Kafka; the subsequent full resnapshot redelivers those keys again. This is within the at-least-once contract, but downstream consumers should be prepared for it.
6. **Causes of "non-trackable," and what the policy actually covers.** Beyond idle TTL expiry (item 1), a CHANGES window also becomes non-trackable when the underlying data changed shape since the base bookmark: a dropped or truncated partition/table, a schema-rewriting `ALTER TABLE` (e.g. a type change requiring a slow schema change), a bucket count or resharding change, a partition/tablet hint that no longer resolves, or a window spanning a period when `enable_change_data_capture` was toggled off and back on. StarRocks reports these in two different shapes — a BE execution-time `CDC-ERROR-<n> (CODE)` error, or an FE planning-time rejection whose message contains the phrase "not trackable" — and `NonTrackableException.classify` recognizes both of those, as well as a stale/expired bookmark reference, routing all of them to the configured `source.nontrackable.policy`. Any *other* SQL error — one `classify` does not recognize — is not covered by this policy at all, whatever `source.nontrackable.policy` says: the table is retried on every poll, with a `source.poll.interval.ms` pause before each retry, and once it has kept failing for `source.poll.retry.timeout.ms` (10 minutes by default) the task fails. That is what turns a permanent error — a dropped column, a revoked privilege, bookmark meta functions switched off — into a FAILED task rather than a RUNNING one that never delivers.
7. **A whole snapshot, or a whole CHANGES window, is materialized in the worker's heap.** Each `poll()` builds a single in-memory batch — there is no batching or pagination — so all the records of an entire snapshot, or of a window that accumulated over a long outage, are held at once and can exhaust the Kafka Connect worker's heap. Bound it by keeping `source.poll.interval.ms` short so each window stays small, and by snapshotting large tables during quiet periods.
8. **Deleting a connector leaves its live bookmarks held until their TTL expires.** There is no unregister step, so the bookmarks a task still holds when the connector is removed keep pinning their versions against vacuum for up to the granted lease (`source.bookmark.ttl.ms`, 7 days by default, capped by the cluster's `bookmark_reference_max_ttl_ms`) — and indefinitely when neither sets a limit. A task *restart* is fine: at start the task lists every reference its holder still has on the table in `information_schema.table_bookmark_references` and takes them all back — the bookmark it resumes from, heads whose windows never became durable, releases a predecessor never got to issue — so they are released by the normal commit cycle instead of waiting out their TTL. A deletion is not: nothing runs afterwards to do that.
...
11. **On Arrow Flight the driver shifts `DATETIME` by the worker's UTC offset; the connector undoes it.** The bundled driver's (18.0.0) timestamp accessor materialises the value through `Timestamp.valueOf(LocalDateTime)`, i.e. the stored digits read in the JVM's default zone whatever `Calendar` is passed -- eight hours early on a UTC+8 worker (reproduced in-process by `ArrowDriverResultSetTest`). On the Arrow transport the connector reverses that step, so both transports print the same string and the smoke test's parity check compares it. The one residual case is a worker in a DST zone during the spring-forward gap hour, where the reversal is not exact; `-Duser.timezone=UTC` on the worker removes even that. `DATE` and the MySQL protocol were never affected.
12. **Under `no_snapshot`, the position before the first durable window lives only in the FE's bookmark references.** If `information_schema.table_bookmark_references` cannot be read at start (an older cluster without it) or the references have expired, the task restarts at the current version and logs a warning; changes in between are skipped.
...
Within `source.poll.interval.ms` (default 5 seconds), the same consumer prints, in order: one `op: "c"` (the insert), an `op: "d"` immediately followed by an `op: "c"` for `id: 3` (the update), and one `op: "d"` (the delete) — closing the loop from a StarRocks table change to a Kafka message.
