...
| `source.snapshot.mode` | String | `initial` | Whether to take an initial snapshot before streaming changes. One of `initial`, `no_snapshot`. |
...
| `source.nontrackable.policy` | String | `fail` | Action to take when a table's CHANGES window becomes non-trackable. One of `fail`, `resnapshot`. |
...
```
Key: `{ "id": 2 }`. If `source.tombstones.on.delete=true`, this record is immediately followed by a tombstone: same key, same topic/partition/offset, but a `null` value and `null` value schema — the usual Kafka Connect signal for downstream log compaction.
...
- **Snapshot-to-incremental handoff.** With `source.snapshot.mode=initial`, the first poll for a table creates a bookmark and streams the table through `[_BOOKMARK_<id>_]`, emitting `op: "r"` rows whose Kafka Connect offset carries `snapshot_done=false`. Only once the snapshot fully finishes does the table's committed offset flip to `snapshot_done=true`, and that same bookmark id becomes the base of the first incremental CHANGES window — so no version is skipped or double-counted between snapshot and streaming. A crash mid-snapshot always resumes with `snapshot_done=false` (or no offset at all), so the entire snapshot is redone from scratch rather than resumed halfway.
...
1. **Idle tables can lose their position.** This version has no bookmark lease renewal: a table that produces no new version for longer than `source.bookmark.ttlms` (default 7 days) has its bookmark expire on the StarRocks side. The next poll's CHANGES read against that stale range then fails as non-trackable, triggering `fail`/`resnapshot`. Tables with bursty or long idle periods should raise `source.bookmark.ttlms` or set `source.nontrackable.policy=resnapshot` to self-heal automatically. (Lease renewal is on the roadmap as a follow-up, not implemented here.)
...
6. **Other causes of "non-trackable" beyond idle TTL expiry:** a dropped or truncated partition/table, a schema-rewriting `ALTER TABLE` (e.g. a type change requiring a slow schema change), a bucket count or resharding change, or a CHANGES window that spans a period when `enable_change_data_capture` was toggled off and back on. All surface the same way — a StarRocks error that `NonTrackableException.classify` recognizes and the task turns into the configured policy.
