...
11. **On Arrow Flight, `DATETIME` values are shifted by the Connect worker's UTC offset.** A `DATETIME` of `2026-08-05 12:34:56` arrives as the instant for that wall clock read in the *Connect worker's* default timezone, not as UTC — eight hours early on a worker running in UTC+8. The offset follows the worker, not the cluster, so two workers in different zones disagree about the same row. `DATE` is unaffected, and the MySQL protocol is unaffected. The cause is on the Arrow side: the BE emits the wall clock as UTC (`TimestampValue::to_unix_microsecond` is Julian-day arithmetic with no timezone input) and `SET time_zone` changes nothing, so the offset is introduced by the driver's timestamp accessor, which materialises the value through a `LocalDateTime` in the JVM default zone whatever `Calendar` the caller passes. Verified against the bundled driver (18.0.0). Because the zone that matters is the worker's, running the worker with `-Duser.timezone=UTC` makes the shift zero. Otherwise use the MySQL protocol for tables with `DATETIME` columns, or correct downstream by the worker's fixed UTC offset.
