...
- **Resetting offsets also releases the FE references.** `DELETE /connectors/{name}/offsets` (connector stopped) makes `alterOffsets` release every bookmark the tasks' holder still references on the reset tables, so the next start begins at the current version instead of resuming from a pinned one. That needs a configuration that parses and an FE that answers; if a release fails the reset is refused and nothing is written. `PATCH` with explicit offsets is validated only: `bookmark_id` must be a non-negative number and `snapshot_done` a boolean, for a table the configuration captures.
