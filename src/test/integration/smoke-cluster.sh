...
# || true: ADMIN SHOW FRONTEND CONFIG needs OPERATE on SYSTEM, and without it this bare assignment
# from a pipeline dies under pipefail with raw mysql stderr, twenty lines before the privilege
# diagnosis written for exactly that case.
run_mode=$(sr_config run_mode || true)
[ "$run_mode" = "shared_data" ] \
  || fail "cluster run_mode is '${run_mode:-unknown}', this connector requires shared_data"
note "run_mode=shared_data"

bm=$(sr_config enable_bookmark_meta_functions || true)
if [ "$bm" != "true" ]; then
...
[ -n "$d_line" ] && [ -n "$c_line" ] && [ "$d_line" -lt "$c_line" ] \
  || fail "UPDATE: delete-before-insert ordering violated for id=2 (d line ${d_line:-?}, c line ${c_line:-?}) — this is the ORDER BY __ROW_VERSION__, __CHANGE_TYPE__ DESC invariant"
...
# sail through both. Prove it is still up before trusting anything the log says.
kill -0 "$worker_pid" 2>/dev/null || fail "the restarted worker is no longer running (pid $worker_pid)"
# connect.log is opened with >> and deliberately spans both worker lifetimes, so >=1 is satisfied
# by a release from before the restart and the inserts below would prove nothing.
released_before=$(grep -c "Released bookmark" "$OUT_DIR/connect.log" || true)
...
# Drive a few more windows, each separated by more than offset.flush.interval.ms, so the flushed
# offset advances and reclamation is forced.
for i in 5 6 7; do
...
[ "${released:-0}" -gt "$released_before" ] \
  || { grep -E "Emitted CDC window|Released bookmark|Failed to release" "$OUT_DIR/connect.log" | tail -20 || true
       fail "no bookmark was released after the restart (still $released_before) — the two-phase fence is not advancing, so every window's bookmark stays pinned against vacuum until its TTL"; }
note "reclamation confirmed ($((released - released_before)) new release(s) after the restart)"
...
# A negative grep passes when the log is missing or empty, and this is the last step in the file.
grep -q "Emitted CDC window" "$OUT_DIR/connect.log" \
  || fail "$OUT_DIR/connect.log holds no 'Emitted CDC window' line; there is nothing to check for failures"
# "Failed to release preflight probe bookmark" splits the two words, so match on the pattern.
if grep -qE "Failed to release .*bookmark" "$OUT_DIR/connect.log"; then
